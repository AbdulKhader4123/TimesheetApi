...
db.mongoose.connection.on("disconnecting", () => {
    console.error("Mongoose default connection is disconnecting");
});
db.mongoose.connection.on("reconnected", () => {
    console.error("Mongoose default connection is reconnected");
});
db.mongoose.connection.on("reconnectFailed", () => {
    console.error("Mongoose default connection reconnectFailed event");
});
db.mongoose.connection.on("reconnectTries", () => {
    console.error("Mongoose default connection reconnectTries event");
});
db.mongoose.connection.on("close", () => {
    console.error("Mongoose default connection close event");
});
db.mongoose.connection.on("error", (err) => {
    console.error(`Mongoose default connection error event: ${JSON.stringify(err)}`);
});
...
const timeSheetTemplateRoute = require("./routes/timesheet-template.route");

const router = express.Router();
router.use("/timeSheets", timeSheetRoute());
router.use("/timeSheetTemplates", timeSheetTemplateRoute());
